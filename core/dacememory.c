...
typedef struct dmem {
    monomial *mem; 
    variable *var; 
    unsigned int nda, mda, lda, nst, lst;
} dacemem;

// Global DACE data structure (only local to this file, not visible outside)
...
void daceMemoryDump()
{
    printf("DACE core memory model: dynamic\n");
...
// Global memory array
typedef struct dmem {
    monomial mem[DACE_STATIC_MEM_SIZE];
    variable var[DACE_STATIC_VAR_SIZE];
    unsigned int nda, mda, nst;
} dacemem;

// Global DACE data structure (only local to this file, not visible outside)
static dacemem DACEMem;
...
void daceAllocateDA(DACEDA *inc, const unsigned int len)
{
    ...
    // Check if we ran out of variables or stack
    if(((DACEMem.nda+1) > DACE_STATIC_VAR_SIZE) || ((DACEMem.nst+ilen) > DACE_STATIC_MEM_SIZE))
    {
...
void daceMemoryDump()
{
    printf("DACE core memory model: static\n");
    printf("Allocated system memory:\n");
    double m = DACE_STATIC_VAR_SIZE*sizeof(variable)/1024.0/1024.0;
    printf("\t# of DAs = %d \t(%d MB)\n", DACE_STATIC_VAR_SIZE, (unsigned int)m);
    m = DACE_STATIC_MEM_SIZE*sizeof(monomial)/1024.0/1024.0;
    printf("\tstack    = %d \t(%d MB)\n", DACE_STATIC_MEM_SIZE, (unsigned int)m);

    printf("Allocated internal memory:\n");
    m = (double)DACEMem.nda/DACE_STATIC_VAR_SIZE*100.0;
    printf("\t# of DAs = %d \t(%d %%)\n", DACEMem.nda, (unsigned int)m);
    m = (double)DACEMem.nst/DACE_STATIC_MEM_SIZE*100.0;
    printf("\tstack    = %d \t(%d %%)\n", DACEMem.nst, (unsigned int)m);

    if(DACE_STATIC_VAR_SIZE > 0)
    {
         printf("Objects:\n");
         unsigned int j = 0, l = 0;
         for( unsigned int i = 0; i < DACEMem.nda; i++ )
...
void daceMemoryDump()
{
    printf("DACE core memory model: system malloc\n");
    return;
}
